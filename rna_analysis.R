...
clean_excel_data <- function(file_path, sheet_name = NULL) {
  # Read the Excel file
  ...
  # Find the row with cytokine names (usually row 3)
  cytokine_row <- which(grepl("IL-|IFN|TNF|MCP|MIP|IP-|KC", df[1:10, 1], ignore.case = TRUE))[1]
  if (is.na(cytokine_row)) cytokine_row <- 3
  
  # Read data with proper headers
  df_clean <- read_excel(file_path, skip = cytokine_row - 1)
  ...
  numeric_cols <- sapply(df_clean, function(x) !is.character(x) | all(!is.na(as.numeric(x))))
...
prepare_analysis_data <- function(data, groups) {
  # Remove non-numeric columns and sample ID column
...
<body>
    ...
    <div class="section">
        ...
        <p>The following cytokines were analyzed: IL-15, IL-17A/F, IL-27p28/IL-30, IL-33, IL-9, IP-10, MCP-1, MIP-1α, MIP-2</p>
    ...
    <div class="section">
        ...
        <p>The following cytokines were analyzed: IFN-γ, IL-10, IL-12p70, IL-1β, IL-2, IL-4, IL-5, IL-6, KC/GRO, TNF-α</p>
