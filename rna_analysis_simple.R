...
# RNA/Cytokine Analysis Script - Simplified and Correct
...
# Function to clean Excel data - Correctly handles the actual data structure
clean_excel_data <- function(file_path) {
  # Read the Excel file, skipping the first 2 rows
  df <- read_excel(file_path, skip = 2)
  
  # The first row (index 0) contains cytokine names
  # The second row (index 1) contains sample IDs
...
create_heatmap <- function(data, group_col = "Group", title = "Cytokine Expression Heatmap") {
  ...
  # Create annotation dataframe with unique row names
  annotation_df <- data.frame(
    Group = groups,
    row.names = make.unique(sample_ids)
  )
  
  # Create color palette
  colors <- brewer.pal(8, "Set2")
  
  # Remove rows with all NA values for heatmap
  na_rows <- rowSums(is.na(numeric_data)) == ncol(numeric_data)
  if (any(na_rows)) {
    numeric_data <- numeric_data[!na_rows, ]
    annotation_df <- annotation_df[!na_rows, , drop = FALSE]
  }
...
create_pca_plot <- function(data, group_col = "Group", title = "PCA Plot") {
  ...
  if (sum(complete_rows) < 2) {
    cat("Warning: Not enough complete cases for PCA in", title, "\n")
    return(NULL)
  }
  
  numeric_data <- numeric_data[complete_rows, ]
  group_data <- data[[group_col]][complete_rows]
...
<body>
    ...
    <div class="section">
        ...
        <ul>
            ...
            <li><strong>Group U:</strong> Samples U001-U026</li>
        </ul>
